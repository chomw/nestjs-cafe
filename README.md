# ☕ Blue Cafe (커뮤니티 플랫폼 백엔드 프로젝트)

Blue Cafe는 네이버 카페의 비즈니스 모델을 모티브로 구현한 커뮤니티 플랫폼 백엔드 서버입니다.

이 프로젝트는 단순한 서비스 클론을 넘어, **현대적인 백엔드 기술 스택을 도입하고 실무 환경을 고려한 아키텍처를 직접 구축해 보는 것**을 최우선 목표로 삼았습니다. 단일 EC2 인스턴스 환경 내에서 자원을 최대한 효율적으로 활용하기 위해 고민했으며, **Nest.js의 체계적이고 객체지향적인 아키텍처**를 기반으로 견고한 시스템 뼈대를 구축했습니다.

특히 단순 기능 구현에 머물지 않고 다음과 같은 인프라 구성 및 성능 고도화에 집중하여 각 기술의 도입 목적을 달성해 내고 있습니다.

* **보안 및 상태 관리:** Redis와 JWT를 활용한 고도화된 세션(RTR) 및 인증/인가 파이프라인
* **웹 서버 및 통신 보안:** Nginx 리버스 프록시 구축 및 Let's Encrypt를 통한 HTTPS(SSL) 암호화 통신 적용
* **데이터베이스 성능 튜닝:** 커버링 인덱스 설계, N+1 쿼리 해결 및 In-Memory 캐싱을 통한 데이터 접근 최적화
...
👉 **[Blue Cafe API 서버 접속하기](https://nestjscafe.com/)**
...
* **Web Server / Security:** Nginx, Let's Encrypt (SSL/TLS)
...
### 3. 웹 서버 및 HTTPS 보안 설정 (Nginx & SSL)

* **리버스 프록시 구축:** 단일 인스턴스 내에서 Nginx를 웹 서버로 도입하여 외부의 80(HTTP), 443(HTTPS) 포트 요청을 전담하게 하고, 이를 내부의 Nest.js 컨테이너(3000 포트)로 안전하게 라우팅하여 외부 직접 접근을 차단했습니다.
* **HTTPS 통신 암호화:** Let's Encrypt(Certbot)를 활용하여 무료 SSL 인증서를 발급 및 자동 갱신하도록 설정하여 클라이언트와 서버 간의 데이터 탈취를 방지하고 신뢰성 있는 통신 환경을 구축했습니다.

### 4. 대용량 미디어 처리 파이프라인 (AWS S3 & CloudFront)
...
### 5. 전역 응답 및 예외 처리 시스템 (Unified Response)
...
### 8. 무중단 CI/CD 자동화 파이프라인 (Docker & GitHub Actions)
